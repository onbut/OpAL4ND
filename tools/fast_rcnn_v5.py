...
def fast_rcnn_inference(boxes, scores, image_shapes, predictions, score_thresh, nms_thresh, topk_per_image, proposal_boxes):
    """
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(
            boxes_per_image, scores_per_image, image_shape, score_thresh, nms_thresh, topk_per_image, prediction, proposal_box_per_image
        )
        for scores_per_image, boxes_per_image, image_shape, prediction, proposal_box_per_image in zip(scores, boxes, image_shapes, predictions, proposal_boxes)
    ]
    return [x[0] for x in result_per_image], [x[1] for x in result_per_image]


def fast_rcnn_inference_single_image(
    boxes, scores, image_shape, score_thresh, nms_thresh, topk_per_image, prediction, proposal_boxes
):
    """
    ...
    proposal_boxes = Boxes(proposal_boxes.reshape(-1, 4))
    proposal_boxes.clip(image_shape)
    proposal_boxes = proposal_boxes.tensor.view(-1, num_bbox_reg_classes, 4)  # R x C x 4
    ...
    if num_bbox_reg_classes == 1:
        ...
        proposal_boxes = proposal_boxes[filter_inds[:, 0], 0]
    else:
...
class PrototypeLocalDecoder(nn.Module):
    ...
    def forward(self, prototype_features, noisy_features):
        delta = noisy_features - prototype_features
        input_features = torch.cat([prototype_features, delta], dim=-1)
        recon = self.decoder(input_features)
        return recon
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # self.hingeloss = nn.HingeEmbeddingLoss(5)
        # self.hingeloss = nn.HingeEmbeddingLoss(10)
        # self.hingeloss = nn.HingeEmbeddingLoss(15)
        # self.hingeloss = nn.HingeEmbeddingLoss(20)
        ...
        self.cls_mean_dir_path = os.path.join(self.output_dir, 'ab_save', 'cls_mean')

        # 如果文件夹不存在咋则创建文件夹
        if not os.path.exists(self.cls_mean_dir_path):
            os.makedirs(self.cls_mean_dir_path, exist_ok=True)
    ...
    def get_clustering_loss(self, input_features, proposals):
        ...
        if storage.iter == self.clustering_start_iter:
            ...
            if storage.iter % self.clustering_update_mu_iter == 0:
                ...
                # # save the means
                torch.save(self.means, os.path.join(self.cls_mean_dir_path, str(storage.iter) + '.pt'))
                # logging.getLogger(__name__).info('Saving means at iteration ' + str(storage.iter) + ' to ' + os.path.join(self.cls_mean_dir_path, storage.iter + '.pt'))
            c_loss = self.clstr_loss_l2_cdist(input_features, proposals)
        return c_loss

    def get_orthogonal_regularization_loss(self, s):  # (Class*K,d)
    ...
    def get_loss_instance(self, input_features, proposals):
        loss_inst = 0
        storage = get_event_storage()
        if storage.iter >= self.clustering_start_iter:
            loss_inst = self.clstr_loss_l2_cdist(input_features, proposals)
        return loss_inst
    
    def get_loss_contrast(self, input_features, proposals):
        loss_contrast = 0
        storage = get_event_storage()
        if storage.iter >= self.clustering_start_iter:
            # 计算两两不同的self.means之间的距离并返回平均值
            all_means = self.means
            for item in all_means:
                if item != None:
                    length = item.shape
            ...
            # 计算两两不同的self.means之间的距离并返回平均值
            distances = torch.cdist(torch.stack(all_means).cuda(), torch.stack(all_means).cuda(), p=self.margin)
            loss_contrast = 0 - distances.mean()

        return loss_contrast
    
    def get_loss_reconstruction(self, input_features, proposals):
        loss_recon = 0
        ...
        if storage.iter >= self.clustering_start_iter:
            ...
            # three_layer_net.apply(Xavier)

            # 将self.means中的每一个作为输入，得到重构的means_recon
            all_means = self.means
            means_recon = all_means
            for item in all_means:
                if item != None:
                    length = item.shape
                    break
            ...
            noise = torch.randn_like(torch.stack(all_means)) * 0.05
            noise_f = noise + torch.stack(all_means)
            
            means_recon = self.three_layer_net(torch.stack(all_means), noise_f)

            # for i, item in enumerate(all_means):
            #     if item is not None:
            #         means_recon[i] = self.three_layer_net(item)
            #         print(self.three_layer_net[i](item).grad)
            #         # means_recon[i] = item
            ...
            for item in means_recon:
                if item != None:
                    length = item.shape
                    break

            for i, item in enumerate(means_recon):
                if item == None:
                    means_recon[i] = torch.zeros((length))

            # 记录每一类的样例的数量
            class_counts = {}

            for index, feature in enumerate(fg_features):
                for cls_index, mu in enumerate(means_recon):
                    if cls_index not in class_counts:
            ...
            #     for cls_index, mu in enumerate(self.means):
            #         if mu is not None and feature is not None:
            #             cls_mark = classes[index].item()
            #             label_new = 1 / class_counts[cls_mark]
            #             if  classes[index] ==  cls_index:
            #                 # label_new 为1/类别的样例数量
            #                 labels.append(1)
            #             else:
            #                 labels.append(-1)
            #         else:
            ...
            for cls_index,mu in enumerate(means_recon):
                num_cls = class_counts[cls_index]

                for index, feature in enumerate(fg_features):
                    if classes[index] == cls_index:
                        # 计算范数:特征向量与重构向量的距离的平方
                        feature_cpu = feature.to("cpu")
                        mu_cpu = mu.to("cpu")
                        loss_recon += (torch.norm(feature_cpu - mu_cpu) ** 2) / num_cls
        return loss_recon
    ...
    def losses(self, predictions, proposals, input_features=None):
        ...
        if input_features is not None:
            ...
            # losses["loss_instance"] = self.get_loss_instance(input_features, proposals) * 0.0000001
            # losses["loss_contrast"] = self.get_loss_contrast(input_features, proposals) * self.loss_alpha
            
            storage = get_event_storage()
            if storage.iter == self.clustering_start_iter:
                # self.three_layer_net = ThreeLayerNet()
                self.three_layer_net = PrototypeLocalDecoder()
                # self.three_layer_net.apply(Xavier)
                self.loss_alpha = nn.Parameter(torch.tensor(1.0))
            if storage.iter >= self.clustering_start_iter:
                losses["loss_reconstruction"] = self.get_loss_reconstruction(input_features, proposals) * self.loss_alpha * 0.001
                if self.num_classes == 2:
                    protos_centers = self.means
                    protos_centers = torch.stack(protos_centers).cuda()
                    losses['loss_orthogonal'] = self.get_orthogonal_regularization_loss(protos_centers) * 0.1
            # losses["loss_prototype"] = self.prototype_loss(input_features, proposals)
    ...
    def inference(self, predictions, proposals):
        ...
        """
        boxes, proposal_boxes = self.predict_boxes(predictions, proposals)
    ...
    def predict_boxes(self, predictions, proposals):
        ...
        # 在不进偏移的情况下，对proposal_boxes进行格式转换
        proposal_boxes_shaped = torch.zeros_like(proposal_deltas)
        proposal_boxes_shaped[:, 0::4] = proposal_boxes[:, 0, None]  # x1
        proposal_boxes_shaped[:, 1::4] = proposal_boxes[:, 1, None]  # y1
        proposal_boxes_shaped[:, 2::4] = proposal_boxes[:, 2, None]  # x2
        proposal_boxes_shaped[:, 3::4] = proposal_boxes[:, 3, None]  # y2
        return predict_boxes.split(num_prop_per_image), proposal_boxes_shaped.split(num_prop_per_image)
